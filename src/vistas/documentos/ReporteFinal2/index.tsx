...
export default function DocumentoReporteFinal2() {
  ...
  useEffect(() => {
    if (deseaDescargarDocumento) {
      (async () => {
        const imagenADocumento: any = document.getElementById('contenido-reporte');
        const canvas = await html2canvas(imagenADocumento, { scale: 2 });
        ...
        const pdf = new jsPDF('p', 'mm', [280, 216]);

        pdf.addImage(divImage, 'JPEG', 0, 0, 216, 280);
        pdf.addImage(firmasImage, 'JPEG', 20, 230, 177, 45);
        pdf.addImage(footerImage, 'JPEG', 68, 255, 80, 10);
  ...
  return (
    <>
      ...
      <div id="capturaReporteFinal2" style={documentStyles} className="pagina">
        <div id="contenido-reporte" style={documentStyles}>
          <Encabezado />
          <br />
          <br />
